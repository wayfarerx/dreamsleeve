...
/**
 * Test case for the copy binary codec.
 */
class CopiesSpec extends FlatSpec with Matchers {

  "A copy operation" should "encode to and decode from binary" in {
    val f = Update.Copy(Value.Boolean())
    val fh = Value.Boolean().hash.toBytes
    val t = Update.Copy(Value.Boolean(true).hash)
    val th = Value.Boolean(true).hash.toBytes
    val codec = Codec[Update.Copy]
