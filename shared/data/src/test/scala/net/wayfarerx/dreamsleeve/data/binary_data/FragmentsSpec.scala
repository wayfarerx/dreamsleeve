...
/**
 * Test case for the fragment binary codec.
 */
class FragmentsSpec extends FlatSpec with Matchers {

  "All fragments" should "encode to and decode from binary" in {
    val b = Value.Boolean(true)
    val n = Value.Number(1)
    val s = Value.String("1")
    val t = Table()
    val codec = Codec[Fragment]
    ...
    codec.encode(t) shouldBe
      Attempt.successful(bin"110000000000000000")
    ...
    codec.decode(bin"110000000000000000") shouldBe
      Attempt.successful(DecodeResult(t, BitVector.empty))
  }
